"""
Molecular Synthesis evaluation using token-level PromptedLLM + AWRS.
...
async def molecular_model_adaptor(instance, output_dir: str, replicate: int) -> ModelOutput:
    ...
    # SMILES grammar constraint coerced to token LM
    condition = PartialSMILES().coerce(TOKEN_LLM, f=b"".join)
...
async def main():
    parser = argparse.ArgumentParser(
        description="Molecular Synthesis eval with token-level model (PromptedLLM + AWRS)"
    )
