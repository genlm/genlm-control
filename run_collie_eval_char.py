"""
COLLIE evaluation using character-level ByteLLM + AWRS.

This script evaluates a model's ability to generate constrained text according
to COLLIE benchmark constraints using byte-level generation.
"""
...
from genlm.control import ByteLLM, AWRS, Potential
...
class CollieConstraintPotential(Potential):
    """Potential wrapper for COLLIE constraints that works with byte sequences."""
    
    def __init__(self, constraint, targets):
        """
        Args:
    ...
    async def complete(self, context):
        ...
        try:
            ...
            if isinstance(context, list):
                ...
                text = context.decode('utf-8', errors='ignore') if isinstance(context, bytes) else str(context)
            # Check constraint
    ...
    async def logw_next(self, context):
        """Compute next-byte log weights.
        
        Since we can't predict constraint satisfaction per byte,
        we allow all bytes equally.
        
        Args:
            context: Current byte sequence
            
        Returns:
            LazyWeights for next bytes
        """
        from genlm.control.util import LazyWeights
        # Allow all bytes equally - constraint checking happens at complete()
        return LazyWeights([0.0] * len(self.vocab_eos))
    
    def coerce(self, llm, f):
        """Coerce to work with byte-level LLM."""
        # Create new instance with LLM's vocabulary
        coerced = CollieConstraintPotential(self.constraint, self.targets)
...
def build_byte_llm(model_name: str = "meta-llama/Llama-3.2-1B-Instruct", K: int = 5) -> ByteLLM:
    """Build a byte-level ByteLLM with specified beam parameters."""
...
async def main():
    parser = argparse.ArgumentParser(
        description="COLLIE eval with character-level model (ByteLLM + AWRS)"
    )
    ...
    parser.add_argument("--K", type=int, default=5, help="Beam size for ByteLLM")
    ...
    global BYTE_LLM
    BYTE_LLM = build_byte_llm(args.model, K=args.K)
    ...
    print(f"n_particles: {args.n_particles}, ess_threshold: {args.ess_threshold}, max_tokens: {args.max_tokens}, K: {args.K}")
    ...
    print("\n" + "=" * 80)
    print(f"Final Results (model={args.model}, ess_threshold={args.ess_threshold}, K={args.K})")
    print("=" * 80)
