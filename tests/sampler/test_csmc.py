...
class _MockModel:
    ...
    :class:`llamppl.Model` (e.g. ``SequenceModel``) uses its start hook
    to establish the initial weight before any step.
...
@pytest.mark.asyncio
async def test_csmc_standard_start_called_once_on_every_particle():
    """Regression: ``csmc_standard`` must run the model's ``start`` hook
    before stepping, exactly as llamppl's ``smc_standard`` and
    ``smc_steer`` do.
...
@pytest.mark.asyncio
async def test_csmc_standard_start_weight_contributes_to_output_weight():
