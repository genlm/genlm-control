...
def test_drive_or_hop_inline_vs_fallback():
    """The burst's inline-or-hop primitive: a non-suspending coroutine is driven
    inline (no event-loop hop); one that hits a real ``await`` raises the dedicated
    ``_CoroutineSuspended`` from ``_drive_sync`` and ``_drive_or_hop`` falls back to
    the hop. Pins the AWRS suspending-condition guard without needing the engine."""
    from genlm.control.sampler.util import (
        _drive_sync,
        _drive_or_hop,
        _CoroutineSuspended,
    )
    ...
    async def non_suspending():
        return 42

    # Inline: result returned, run_async never called.
    assert _drive_or_hop(non_suspending, run_async) == 42
    assert hops == []
    ...
    # _drive_sync raises the dedicated sentinel (not a bare RuntimeError).
    with pytest.raises(_CoroutineSuspended):
        _drive_sync(suspending())

    # _drive_or_hop catches it and re-runs a fresh coroutine on the loop.
    assert _drive_or_hop(suspending, run_async) == 7
    assert len(hops) == 1
